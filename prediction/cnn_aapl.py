    ...
    for stock_name in stocks_name:
        for relate in related_type:
            for senti in sentiment_type:
                ...
                # 构建滑动窗口数据
                predicted_data = []
                actual_data = []#len(data) - window_size -his_long- 1
                for i in range(len(data) - window_size -his_long- 1): 
                    x = []
                    y = []

                    for j in range(his_long - window_size -1):
                        x.append(data[i+j:i+j+window_size, :size])  # use all 9 columns as input
                    ...
                    class CNNModel(nn.Module):
                        ...
                        def __init__(self, ninput, noutput):
                            ...
                            self.layer1 = nn.Sequential(
                                nn.Conv1d(ninput, 16, kernel_size=5, stride=1, padding=2),  # Using 1D convolutions
                                nn.ReLU(),
                            ...
                            self.fc = nn.Linear(32 * (window_size // 4), noutput)
                        
                        def forward(self, x):
                            x = x.permute(0, 2, 1)  # CNN expects batch_size, num_features, sequence_length
                            out = self.layer1(x)
                            out = self.layer2(out)
                    ...
                    # 创建模型
                    model = CNNModel(ninput=size, noutput=size)
