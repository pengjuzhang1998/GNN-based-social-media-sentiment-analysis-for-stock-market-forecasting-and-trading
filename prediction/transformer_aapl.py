    ...
    for stock_name in stocks_name:
        for relate in related_type:
            for senti in sentiment_type:
                if relate == 'related':
                    csv_name = 'transformer/transformer_predict_'+stock_name+'_'+senti+str(window_size)+'.csv'
                else:
                    csv_name = 'transformer/transformer_predict_'+stock_name+'_'+senti+'_'+relate+str(window_size)+'.csv'
                ...
                for i in range(len(data) - window_size -his_long- 1): 
                    ...
                    class TransformerModel(nn.Module):
                        ...
                        def __init__(self, ninput, nhead, nhid, nlayers, noutput):
                            super(TransformerModel, self).__init__()
                            self.transformer_encoder = nn.TransformerEncoder(
                                nn.TransformerEncoderLayer(d_model=ninput, nhead=nhead, dim_feedforward=nhid), 
                                num_layers=nlayers
                            )
                        ...
                        def forward(self, x):
                            ...
                            out = self.fc(out[-1, :, :])  # We want the last timestep
                            return out
                    num_epochs = 100

                    # 创建模型
                    model = TransformerModel(ninput=size, nhead=size, nhid=64, nlayers=2, noutput=size)
